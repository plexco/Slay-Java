...
execute if score @e[tag=selected,limit=1] group = @s group if score #amount tmp matches 2.. if score @s[tag=moveable] handItem matches 2..5 run scoreboard players operation @p[tag=currentPlayer] handItem = @s handItem 
execute if score @e[tag=selected,limit=1] group = @s group if score #amount tmp matches 2.. if score @s[tag=moveable] handItem matches 2..5 run function slay:utils/remove/allunits 
